...
@router.post("/agent_trainer_qna")
async def agent_trainer_qna(assistant: schema.TrainingQnaAssistant, request: Request, db: Session = Depends(get_db)):
    ...
    # Add message to conversation
    message = models.MessageTraining(conversation_training_id=assistant.conversation_id, content=assistant.question, role=RoleType.human)
    db.add(message)
    db.commit()

    conversation = db.query(models.ConversationTraining).filter(models.ConversationTraining.id == assistant.conversation_id).first()
    if not conversation:
        raise HTTPException(status_code=404, detail="Conversation not found")
    
    llm_response = llm_helper.agent_trainer_qna_response(assistant.question, character=conversation.character_id)

    
    print(repr(llm_response))
    message = models.MessageTraining(conversation_training_id=assistant.conversation_id, content=llm_response["completion"], role=RoleType.bot)
    db.add(message)
    db.commit()
    
    return JSONResponse(
        status_code=201,
        content={"completion": llm_response["completion"]
                 }
    )
